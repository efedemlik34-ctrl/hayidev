...
module.exports = function(ctx) {
  ...
  // ═══════════════════════════════════════════════════════════════
  //  EKLENEN ENDPOINT'LER (auto-injected)
  // ═══════════════════════════════════════════════════════════════
  try {
    ...
    app.post('/api/user/update', auth, (req, res) => {
      try {
        ...
        db.prepare('UPDATE users SET bio=COALESCE(?,bio), gender=COALESCE(?,gender), age=COALESCE(?,age), country=COALESCE(?,country) WHERE id=?')
          .run(bio, gender, age, country, req.user.id);
        res.json({ ok: true });
    ...
    app.get('/api/friends', auth, (req, res) => {
      try {
        res.json(db.prepare(`SELECT u.id, u.username, u.level, u.vip FROM friends f JOIN users u ON u.id = f.friend_id WHERE f.user_id = ? AND f.status = 'accepted'`).all(req.user.id));
      } catch (_) { res.json([]); }
    });

    app.get('/api/friends/requests', auth, (req, res) => {
      try {
        res.json(db.prepare(`SELECT u.id, u.username, u.level FROM friends f JOIN users u ON u.id = f.user_id WHERE f.friend_id = ? AND f.status = 'pending'`).all(req.user.id));
      } catch (_) { res.json([]); }
    });
    ...
    app.post('/api/friends/accept', auth, (req, res) => {
      try {
        const { userId } = req.body;
        db.prepare('UPDATE friends SET status = ? WHERE user_id = ? AND friend_id = ?').run('accepted', userId, req.user.id);
      } catch (_) {}
      res.json({ ok: true });
    });
    ...
    app.get('/api/dm/:userId', auth, (req, res) => {
      try {
        const me = req.user.id, other = parseInt(req.params.userId);
        res.json(db.prepare(`SELECT m.*, u.username as sender FROM direct_messages m LEFT JOIN users u ON u.id = m.sender_id WHERE (m.sender_id = ? AND m.receiver_id = ?) OR (m.sender_id = ? AND m.receiver_id = ?) ORDER BY m.id ASC LIMIT 200`).all(me, other, other, me));
      } catch (_) { res.json([]); }
    });

    app.get('/api/dm/messages/:otherId', auth, (req, res) => {
      try {
        const me = req.user.id, other = parseInt(req.params.otherId);
        res.json(db.prepare(`SELECT * FROM direct_messages WHERE (sender_id = ? AND receiver_id = ?) OR (sender_id = ? AND receiver_id = ?) ORDER BY id ASC LIMIT 200`).all(me, other, other, me));
      } catch (_) { res.json([]); }
    });
    ...
    app.post('/api/dm/send', auth, (req, res) => {
      try {
        ...
        db.prepare('INSERT INTO direct_messages (sender_id, receiver_id, text) VALUES (?, ?, ?)').run(req.user.id, receiverId, text);
      } catch (_) {}
      res.json({ ok: true });
    });
    ...
    app.get('/api/follow/followers', auth, (req, res) => {
      try {
        res.json(db.prepare(`SELECT u.id, u.username, u.level FROM follows f JOIN users u ON u.id = f.follower_id WHERE f.following_id = ?`).all(req.user.id));
      } catch (_) { res.json([]); }
    });
    ...
    app.get('/api/clans/my', auth, (req, res) => {
      try {
        const c = db.prepare('SELECT * FROM clans WHERE owner_id = ? LIMIT 1').get(req.user.id);
        res.json(c || {});
      } catch (_) { res.json({}); }
    });

    app.get('/api/clans/:id/war', auth, (req, res) => res.json({ active: false, points: 0, enemies: [] }));
    app.get('/api/clans/war/points', auth, (req, res) => res.json({ points: 0 }));

    // GROUPS
    app.get('/api/groups/my', auth, (req, res) => {
      try { res.json(db.prepare('SELECT * FROM groups WHERE owner_id = ?').all(req.user.id)); }
      catch (_) { res.json([]); }
    });
    ...
    app.post('/api/groups/create', auth, (req, res) => {
      try {
        const { name } = req.body;
        const info = db.prepare('INSERT INTO groups (name, owner_id) VALUES (?, ?)').run(name, req.user.id);
    ...
    app.post('/api/notifications/read', auth, (req, res) => res.json({ ok: true }));

    // POSTS
    app.get('/api/posts', auth, (req, res) => {
      try { res.json(db.prepare('SELECT * FROM posts ORDER BY id DESC LIMIT 50').all()); }
      catch (_) { res.json([]); }
    });
    ...
    app.post('/api/posts', auth, (req, res) => {
      try {
        const { text } = req.body;
        const info = db.prepare('INSERT INTO posts (user_id, username, text) VALUES (?, ?, ?)').run(req.user.id, req.user.username, text);
        res.json({ ok: true, id: info.lastInsertRowid });
      } catch (_) { res.json({ ok: true }); }
    });

    app.post('/api/posts/:id/like', auth, (req, res) => res.json({ ok: true }));

    // STORIES
    app.get('/api/stories', auth, (req, res) => {
      try { res.json(db.prepare('SELECT * FROM stories ORDER BY id DESC LIMIT 30').all()); }
      catch (_) { res.json([]); }
    });
    ...
    // PROFILE VISITORS
    app.get('/api/profile/visitors', auth, (req, res) => {
      try { res.json(db.prepare('SELECT * FROM profile_visitors WHERE visited_id = ? ORDER BY id DESC LIMIT 50').all(req.user.id)); }
      catch (_) { res.json([]); }
    });

    // GAMES
    const __gameHandler = (game, chance, mult) => (req, res) => {
      try {
        const { bet } = req.body;
        ...
        db.prepare('UPDATE users SET balance = balance - ? WHERE id = ?').run(bet, req.user.id);
        const won = Math.random() < chance;
        const win = won ? Math.floor(bet * mult) : 0;
        if (win > 0) db.prepare('UPDATE users SET balance = balance + ? WHERE id = ?').run(win, req.user.id);
        const nb = db.prepare('SELECT balance FROM users WHERE id = ?').get(req.user.id).balance;
        res.json({ won, win, newBalance: nb, bet });
      } catch (e) { res.status(500).json({ error: e.message }); }
    };

    app.post('/api/dragon-tiger/bet', auth, __gameHandler('dragon_tiger', 0.45, 2));
    app.post('/api/bull-cowboy/bet', auth, __gameHandler('bull_cowboy', 0.45, 2));
    app.post('/api/football/bet', auth, __gameHandler('football', 0.45, 2));
    app.post('/api/teen-patti/bet', auth, __gameHandler('teen_patti', 0.42, 3));
    app.post('/api/rocket/bet', auth, __gameHandler('rocket', 0.5, 2));
    app.post('/api/rocket/cashout', auth, (req, res) => res.json({ ok: true }));
    app.post('/api/golden-fortune/spin', auth, __gameHandler('golden_fortune', 0.35, 5));
    app.post('/api/greedy-pro/spin', auth, __gameHandler('greedy_pro', 0.4, 3));
    app.post('/api/lucky-fruit/spin', auth, __gameHandler('lucky_fruit', 0.4, 3));
    app.post('/api/lucky-pro/spin', auth, __gameHandler('lucky_pro', 0.38, 4));
    app.post('/api/slot-food/spin', auth, __gameHandler('slot_food', 0.42, 3));
    app.post('/api/jackpot-eagle/spin', auth, __gameHandler('jackpot_eagle', 0.38, 4));
    app.post('/api/jackpot/chest', auth, __gameHandler('jackpot_chest', 0.4, 3));
    app.post('/api/games/bingo/play', auth, __gameHandler('bingo', 0.4, 3));
    app.post('/api/games/teen-patti/play', auth, __gameHandler('teen_patti', 0.42, 3));
    app.post('/api/games/dice/roll', auth, __gameHandler('dice', 0.45, 2));
    app.post('/api/games/coinflip/flip', auth, __gameHandler('coinflip', 0.48, 2));
    app.post('/api/games/dragon-tiger/play', auth, __gameHandler('dragon_tiger', 0.45, 2));
    app.post('/api/games/roulette/spin', auth, __gameHandler('roulette', 0.48, 2));
    app.post('/api/games/slot/spin', auth, __gameHandler('slot', 0.4, 3));
    app.post('/api/games/rocket/start', auth, __gameHandler('rocket', 0.5, 2));
    app.post('/api/games/rocket/cashout', auth, (req, res) => res.json({ ok: true }));

    // VIP
    app.get('/api/svip/my', auth, (req, res) => res.json({ tier: 0 }));
    app.get('/api/svip/tiers', auth, (req, res) => res.json([
    ...
    app.get('/api/flash-sale/active', auth, (req, res) => res.json({ active: false, items: [] }));

    // THEMES
    app.get('/api/themes', auth, (req, res) => {
      try { res.json(db.prepare('SELECT * FROM themes').all()); }
      catch (_) { res.json([]); }
    });

    // TOURNAMENTS
    app.get('/api/tournaments/:id/bracket', auth, (req, res) => res.json({ rounds: [] }));
    app.post('/api/tournaments/:id/join', auth, (req, res) => {
      try { db.prepare('UPDATE tournaments SET players = players + 1 WHERE id = ?').run(req.params.id); }
      catch (_) {}
      res.json({ ok: true });
    });

    // SEASONS
    app.get('/api/seasons/current', auth, (req, res) => res.json({ season: 1, name: 'Sezon 1', endsAt: Date.now() + 30 * 24 * 60 * 60 * 1000 }));
    app.get('/api/daily/7day', auth, (req, res) => {
    ...
    // GIFT BOX
    app.get('/api/gift-box/status', auth, (req, res) => res.json({ available: true, nextIn: 0 }));
    app.post('/api/gift-box/open', auth, (req, res) => {
      const rewards = [100, 200, 500, 1000, 2000];
      const reward = rewards[Math.floor(Math.random() * rewards.length)];
      try { db.prepare('UPDATE users SET balance = balance + ? WHERE id = ?').run(reward, req.user.id); }
      catch (_) {}
    ...
    // AI
    app.post('/api/ai/moderate', auth, (req, res) => res.json({ ok: true, safe: true }));

    console.log('[EXTENDED] 30+ endpoint yuklendi (extra.js icinden)');
  } catch (e) {
